        ...
        try {
            ...
            if (result.success) {
                ...
                setSummary((prev) => ({
                    ...prev,
                    registryCount:
                        issue.type === 'registry' ? prev.registryCount - 1 : prev.registryCount,
                    fileCount: issue.type === 'file' ? prev.fileCount - 1 : prev.fileCount,
                    uninstallerCount:
                        issue.type === 'uninstaller'
                            ? prev.uninstallerCount - 1
                            : prev.uninstallerCount
                }))
            }
        } catch (error) {
            ...
            <div className="issues-container">
                {issues.map((issue, index) => (
                    <div className="issue-card" key={index}>
                        ...
                        <div className="file-path">{issue.path}</div>
                        <button
